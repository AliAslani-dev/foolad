            ...
            <BRow class="g-3">
              ...
              <BCol class="col-lg-6">
                <div class="form-group">
                  ...
                  <input
                    v-model="en_subject"
                    @input="clearError('en_subject')"
                    type="text"
                    class="form-control"
                    placeholder="موضوع را به انگلیسی وارد نمایید"
                  />
                  <small v-if="errors.en_subject" class="text-danger">{{
                    errors.en_subject
                  }}</small>
                </div>
              </BCol>
...
export default {
  setup(props, { emit }) {
    ...
    const validateForm = () => {
      errors.value = {};
      if (!subject.value)
        errors.value.subject = "وارد کردن موضوع ضروری می باشد";
      if (!en_subject.value)
        errors.value.en_subject = "وارد کردن موضوع به انگلیسی ضروری می باشد";
    ...
    const addSubject = () => {
      ...
      formData.append("subject_en", en_subject.value);

      axios
        .post(`${url}/admin/add-subject`, formData, {
          headers: {
        ...
        .catch((error) => {
          console.error(error);
          toast.error("مشکلی در ایجاد موضوع به وجود آمد !", {
            position: "top-right",
            autoClose: 1000,
            onClose: () => emit("subject-updated"),
          });
